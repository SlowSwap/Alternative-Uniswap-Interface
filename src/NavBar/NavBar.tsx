...
export default function NavBar() {
  ...
  return (
    <nav>
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-slate-50 text-opacity-70">
        <img
        ...
        <ul className="mt-2 mb-3 text-center text-xl  list-none flex items-center justify-center gap-8">
          {MenuItems.map((item, index) => {
            return (
              <li key={index}>
                <Link
                  className={"font-medium hover:text-white " + (location.pathname === item.url ? " underline" : "")}
                  to={item.url}
                >
                  {item.title}
                </Link>
